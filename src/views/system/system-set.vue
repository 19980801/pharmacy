      ...
      <p slot="title">系统参数配置</p>
                ...
                <div class="sear">
                    <Form :label-width="100" style="width:300px">
                        <FormItem label="用户分类：" class="searchInput">
                            <Select v-model="userCategoryId" @on-change="showContent">
        ...
        <Modal v-model="addContentModal" :title="modalName" :closable="false">
            <Form ref="formValidate1" :model="formValidate1" :label-width="100" :rules="ruleValidate1">
                <FormItem label="用户分类：" v-if="modalName=='新增课程内容分类'">
...
<script>
import {getTalbe,updateUser,updateQuestion,updateCourse,addUser,addQuestion,addCourse,addCourseContent,updateCourseContent} from "@/service/systemApi/api";
export default {
  data() {
    return {
      userCategoryId:"",
                    ...
                    h(
                        "Button",
                        {
                            ...
                            on: {
                                click: () => {
                                    console.log(1)
                                    this.formValidate.categoryName=params.row.categoryName;
                    ...
                    h(
                        "Button",
                        {
                            ...
                            on: {
                                click: () => {
                                    console.log(1)
                                    this.formValidate1.contentName=params.row.contentName;
  ...
  methods: {
    ...
    // 对应课程分类下的内容分类
    showContent(){
        console.log(this.userCategoryId);
        let arr=[];
        this.typeList.forEach(ele=>{
            if(ele.id==this.userCategoryId){
                arr.push(...ele.courseContentList);
            }
        })
        this.coursepage=arr;
    },
    ...
    sureContent(){
        ...
        if (valid) {
            // 修改
            if(this.isEdit){
                updateCourseContent({...this.formValidate1}).then(res=>{
                       if(res.code==0){
                           for(let key in this.formValidate1){
                                this.formValidate1[key]="";
                            }
