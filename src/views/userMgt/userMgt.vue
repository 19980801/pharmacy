<template>
  <div>
    ...
    <Modal v-model="sendModal" title="发送通知" :closable="false">
      <Form ref="sendValidate" :model="sendValidate" :label-width="120" :rules="rulesSend">
        ...
        <FormItem label="通知内容：" prop="informContent">
          <Input v-model="sendValidate.informContent" type="textarea" :autosize="{minRows:6,maxRows: 5}"></Input>
        </FormItem>
      </Form>
      <div slot="footer">
    ...
    <!-- 修改用户 -->
    <Modal v-model="editModal" title="编辑用户" :closable="false">
      <Form ref="editInfo" :model="editInfo" :label-width="90">
        <FormItem label="姓名：">
          <Input v-model="editInfo.realName"></Input>
        </FormItem>
        <FormItem label="用户名：">
          <Input v-model="editInfo.userName"></Input>
        </FormItem>
        <FormItem label="身份证号：">
          <Input v-model="editInfo.idNumber"></Input>
        </FormItem>
      </Form>
      <div slot="footer">
...
export default {
  data() {
    return {
      editModal: false,
      uploadUrl: `${this.host}/admin/user/import`,
      uploadData: {},
      uploadLoading: false,
      selectedArr: [],
      sendModal: false,
            ...
            return [
              ...
              h(
                "Button",
                {
                  ...
                  on: {
                    click: () => {
                        const {id,realName,userName,idNumber}=params.row;
                        this.editInfo={id,realName,userName,idNumber};
                        this.editModal=true;
                    }
                  }
                },
